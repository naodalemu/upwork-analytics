...
export function Dashboard({ data, onReset }: DashboardProps) {
  ...
  const [dateFilter, setDateFilter] = useState<string>("all") // For predefined ranges
  const [customDateRange, setCustomDateRange] = useState<DateRange | undefined>(undefined) // For custom range
  ...
  const handlePredefinedDateChange = (value: string) => {
    setDateFilter(value)
    setCustomDateRange(undefined) // Clear custom range when a predefined range is selected
  }

  const handleCustomDateRangeChange = (range: DateRange | undefined) => {
    setCustomDateRange(range)
    if (range && range.from) {
      setDateFilter("custom") // Set predefined filter to 'custom' if a custom range is active
    } else {
      setDateFilter("all") // Revert to 'all' if custom range is cleared
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <Button variant="ghost" size="sm" onClick={onReset} className="text-gray-600 hover:text-gray-900">
                <ArrowLeft className="w-4 h-4 mr-2" />
                Back to Upload
              </Button>
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Financial Dashboard</h1>
                <p className="text-gray-600">Analyzing {data.length} transactions</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                ...
                <Select value={dateFilter} onValueChange={handlePredefinedDateChange}>
              ...
              <DateRangePicker date={customDateRange} setDate={handleCustomDateRangeChange} />
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Navigation */}
      <DashboardNavigation currentPage={currentPage} onPageChange={setCurrentPage} />
